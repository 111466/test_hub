...
- **图集与序列帧支持：通过 `manifest.json` 自动解析并裁剪大图 (Tileset)，支持配置动图帧率**
- 瓦片水平翻转（F 键切换，位编码透明支持撤销/剪贴板/填充）
- 瓦片名称和标签自定义
- 撤销/重做（无限步数）
- 选区复制/粘贴
- JSON 导入/导出
...
| `EditorUI.lua` | ~2025 | UI 布局（工具栏、调色板、图层面板、状态栏、弹窗） |
| `IsoCanvas.lua` | ~1020 | 等距画布渲染与交互（NanoVG 绘制、鼠标/键盘操作） |
| `MapData.lua` | ~1700 | 地图数据管理（图层、瓦片、存档、撤销重做、导入导出） |

此外需要：
- 瓦片图片资源放入 `assets/` 下某个文件夹（如 `assets/Tiles/`），PNG 格式
- （可选）在瓦片文件夹中创建 `manifest.json` 以支持图集（Tileset）或序列帧动画
...
如果你的瓦片是一张大图合集 (Tileset) 或需要序列帧动画，请在资源文件夹下创建 `manifest.json`，格式如下：
...
[
  ...
  {
    "type": "tileset",
    "image": "world_tiles.png",
    "tiles": [
      { "name": "草地切片1", "x": 0, "y": 0, "w": 128, "h": 128 },
      { 
        "name": "水波图集动画",
        "fps": 6,
        "frames": [
...
瓦片 ID 约定：`0`=空，`1-5`=颜色瓦片，`100+`=图片瓦片。bit 14 (0x4000) 为水平翻转标志位。
