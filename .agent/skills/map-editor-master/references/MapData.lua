...
-- ============================================================================
-- 翻转标志位编码（bit 15 = flipH，低 14 位 = baseID）
-- 撤销/重做、剪贴板、洪水填充均操作原始整数，无需修改
-- ============================================================================
MapData.FLIP_H_BIT   = 0x4000   -- bit 14 (第15位) 水平翻转标志
MapData.BASE_ID_MASK  = 0x3FFF   -- 低 14 位掩码（最大 baseID = 16383）

--- 提取 baseID（去掉翻转标志）
---@param tileID number
---@return number baseID
function MapData.GetBaseID(tileID)
    return tileID & MapData.BASE_ID_MASK
end

--- 检查是否水平翻转
---@param tileID number
---@return boolean
function MapData.IsFlippedH(tileID)
    return (tileID & MapData.FLIP_H_BIT) ~= 0
end

--- 编码 tileID（baseID + 翻转标志）
---@param baseID number
---@param flipH boolean
---@return number tileID
function MapData.EncodeID(baseID, flipH)
    if flipH then
        return (baseID & MapData.BASE_ID_MASK) | MapData.FLIP_H_BIT
    else
        return baseID & MapData.BASE_ID_MASK
    end
end
...
--- 判断瓦片是否为图片瓦片（自动剥离翻转标志）
---@param tileID number
---@return boolean
function MapData.IsImageTile(tileID)
    return (tileID & MapData.BASE_ID_MASK) >= MapData.IMAGE_TILE_BASE
end
...
--- 翻转指定位置的瓦片（带撤销支持）
---@param x number 地图坐标 X
---@param y number 地图坐标 Y
---@param layerIndex number|nil 层索引
---@return boolean success
function MapData.FlipTileH(x, y, layerIndex)
    ...
    if not layer or layer.locked then return false end
    local oldID = layer.data[y][x]
    if oldID == 0 then return false end
    -- 切换 flipH 标志位
    local newID = oldID ~ MapData.FLIP_H_BIT  -- XOR 翻转
    layer.data[y][x] = newID
    recordChange(layerIndex, x, y, oldID, newID)
    return true
end
...
function MapData.GetTileType(tileID)
    local baseID = tileID & MapData.BASE_ID_MASK
    return MapData.TILE_TYPES[baseID] or MapData.TILE_TYPES[0]
end
    ...
    for li = 1, #MapData.layers do
        ...
        if layer.visible ~= false then
            ...
            if tileID > 0 then
                ...
                -- 2) 再检查瓦片自身 tag（剥离翻转标志）
                local tt = MapData.TILE_TYPES[tileID & MapData.BASE_ID_MASK]
