...
```jsonc
{
  ...
  "imageRegistry": [               // 图片瓦片定义列表
    ...
    },
    {
      "id": 101,
      "name": "grass_tile",
      "imagePath": "Tiles/world_tiles.png",
      "region": { "x": 0, "y": 0, "w": 128, "h": 128 } // 图集子区域裁剪（可选）
    },
    {
      "id": 102,
      "name": "water_anim",
      "imagePath": "Tiles/world_tiles.png",
      "isAnimated": true,          // 是否为序列帧动图
      "fps": 6,                    // 播放帧率
      "frames": [                  // 动画帧数组（可以是单图路径，或包含图集 region 的对象）
        { "imagePath": "Tiles/world_tiles.png", "region": { "x": 0, "y": 128, "w": 128, "h": 128 } },
        { "imagePath": "Tiles/world_tiles.png", "region": { "x": 128, "y": 128, "w": 128, "h": 128 } }
      ]
    }
  ],
  ...
  "layers": [
    {
      ...
      "tiles": [                   // 非空瓦片列表（稀疏存储）
        ...
        { "x": 8, "y": 3, "id": 100, "path": "Tiles/tree.png", "flipH": true }, // 水平翻转
        { "x": 9, "y": 4, "id": 101, "path": "Tiles/world_tiles.png", "region": { "x": 0, "y": 0, "w": 128, "h": 128 } }, // 图集切片
        { "x": 10, "y": 5, "id": 102, "isAnimated": true, "fps": 6, "frames": [...] } // 序列帧动图
      ]
    }
  ]
}
...
2. 建立 id → tileData 的映射表 (包含 imagePath, region, isAnimated, frames 等信息)
3. 遍历 layers[].tiles 时，遇到 id >= 100 的瓦片：
   - 优先使用 tile 自身的 path、region 等字段
   - 若无，则从 imageRegistry 映射表查找
4. 渲染时：
   - 如果是静态图集切片 (存在 region)，则通过裁剪方式绘制该区域。
   - 如果是序列帧动图 (isAnimated 为 true)，根据全局时间累加和 fps 计算当前应当渲染哪一帧的 imagePath/region。
```
