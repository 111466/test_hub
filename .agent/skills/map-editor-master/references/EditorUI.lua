...
function EditorUI.HandleKeyDown(key)
    ...
    if ctrl then
        ...
        elseif key == KEY_F then
            -- F: 翻转鼠标悬停位置的瓦片
            local hx, hy = IsoCanvas.GetHoverCoord()
            if hx and hy and MapData.InBounds(hx, hy) then
                if MapData.FlipTileH(hx, hy) then
                    EditorUI.ShowToast("翻转瓦片")
                else
                    EditorUI.ShowToast("该位置无瓦片")
                end
            end
