...
- （可选）若需使用图集（Tileset）或动图，需在瓦片文件夹中提供 `manifest.json`
...
- **若文件夹中存在 `manifest.json`，则按照配置自动切割大图（Tileset）及识别序列帧动画**
- 以缩略图网格展示，点击选择
- 支持自定义名称和标签
...
```jsonc
{
  ...
  "imageRegistry": [
    ...
    { "id": 101, "name": "grass_tile", "imagePath": "Tiles/world.png", "region": {"x":0,"y":0,"w":128,"h":128} }
  ],
  "tileCustomizations": [
...
**Q: 图片瓦片没有显示？**
A: 检查 `tileFolder` 配置路径是否正确，确保图片文件为 PNG 格式且放在 `assets/` 对应子目录下。如果是通过图集加载，检查 `manifest.json` 内部的图片路径和裁剪坐标是否越界。

**Q: 瓦片图片有几千张，放在一个文件夹太卡怎么办？**
A: 强烈建议将小图合并为一张或几张大图（Sprite Sheet），然后在资源文件夹内编写 `manifest.json` 声明类型为 `tileset` 并指定子切片坐标（`x, y, w, h`）。编辑器会自动按照配置裁剪大图渲染，避免读取成千上万个零散文件。
